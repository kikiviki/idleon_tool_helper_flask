...
def predict():
    if request.method == 'POST':
        ...
        output_array = mainroutine(text)
        # do your prediction here
        #render_template('index.html', parsed_output=text)
    return render_template('output.html', parsed_output=output_array)
...
def mainroutine(json_data):
    ...
    for i in range(10): #iterate through all charcters
        ...
        if use_names:
            final_output.append(character_names_index[i])
        else:
            char_name_tmp = ("Character #:" + str(i))
            final_output.append(char_name_tmp)
        ...
        for j in range(6):
            ...
            final_output.append(character_string)
